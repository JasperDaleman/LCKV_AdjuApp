...
class StafhapEnPB(QWidget):
    def __init__(self, master):
        ...
        self.stafhap_radiobtn.toggled.connect(
            lambda: self.onclick(self.stafhap_radiobtn, -1)
        )
    ...
    def staf_toevoegen(self):
        ...
        for i in range(self.countStaf):
            ...
            self.radiobtn_yes[i].toggled.connect(
                lambda i=i, btn_idx=i: self.onclick(self.radiobtn_yes[i], btn_idx)
            )
            self.radiobtn_no[i].toggled.connect(
                lambda i=i, btn_idx=i: self.onclick(self.radiobtn_no[i], btn_idx)
            )
        ...
        if int(self.bonnr.text()) in self.master.master.bonnen.index:
            ...
            if (
                ...
                msgYesNo.setText(
                    ...
                    + " '"
                    + self.bonomsc
                    + "' is al verdeeld. Opnieuw verdelen?"
                )
    ...
    def bevestig_stafhap_pb(self):
        ...
        if self.bedragpp == 0:
            msgOk.setText(
                "Het te verdelen bedrag is € 0,00. Wijs de bon aan één of meerdere stafleden toe."
            )
        ...
        # check of alle stafleden van deze bon inleg hebben.
            ...
            msgOk.setText(
                self.persZonderInleg + " heeft geen inleg. Voeg deze eerst toe."
            )
        ...
        # Opnieuw verdelen?
            ...
            self.master.master.stafhapPB.Bonnummer.astype(str)
            .str.contains(self.cur_bonnr)
            .any()
        ):
            bonomsc = self.master.master.bonnen.loc[
                float(self.cur_bonnr), "Omschrijving"
            ]
            ...
            msgYesNo.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            msgYesNo.setText(
                "Je gaat de bon opnieuw verdelen. De oude verdeling wordt verwijderd. Doorgaan?"
            )
            msgYesNo.setWindowTitle("Opnieuw verdelen")
            ...
            # Bij oppnieuw verdelen, eerst de oude waardes verwijderen
            if retval == QMessageBox.Yes:
                self.master.master.stafhapPB = self.master.master.stafhapPB[
                    self.master.master.stafhapPB["Bonnummer"] != int(self.bonnr.text())
                ]
                self.verdeel_bon()
        else:
            self.verdeel_bon()
    ...
    def verdeel_bon(self):
        ...
        for idx, row in self.stafledenDF.loc[
            ...
            self.master.master.stafhapPB = self.master.master.stafhapPB.append(
    ...
    def onclick(self, btn, btn_idx):
        ...
        if btn.text() == "PB":
            ...
            if self.radiobtn_yes[btn_idx].isChecked():
                self.stafledenDF.loc[
                    self.stafledenDF["Radiobutton"] == btn_idx, "Deelname"
                ] = True
                self.countStaf += 1
