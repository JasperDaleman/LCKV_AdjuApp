import PyQt5
from PyQt5.QtWidgets import QWidget, QPushButton, QComboBox, QLineEdit, QGridLayout, QTableWidget, QMessageBox
...
class ArtikelBeheer(QWidget):
    ...
    def fill_table(self):
        ...
        for i, row in self.master.master.kantine.iterrows():
            ...
            self.table.setItem(rowCount, 1, QtWidgets.QTableWidgetItem(str(row.Productomschrijving)))  
            self.table.setItem(rowCount, 2, QtWidgets.QTableWidgetItem(str(row.Productcode)))
            ...
            self.table.setItem(rowCount, 6, QtWidgets.QTableWidgetItem(str(row.Calorieen)))
            rowCount += 1
    ...
    def on_changed(self):
        ...
        if self.master.master.kantine.Productcode.str.contains(self.table.currentItem().text()).any():
            curProdcode = self.table.item(self.table.currentRow(), self.table.currentColumn()).text()
            self.msgWarn.setText("Productcode bestaat al. Aanpassen mislukt.")
            self.msgWarn.setWindowTitle("Aanmaken mislukt")
            self.msgWarn.show()
            self.table.item(self.table.currentRow(), self.table.currentColumn()).setText(self.master.master.kantine.iloc[self.table.currentRow(), self.table.currentColumn()])
        elif self.table.currentColumn() == 3 or self.table.currentColumn() == 4 or self.table.currentColumn() == 5 or self.table.currentColumn() == 6:
            self.master.master.kantine.iloc[self.table.currentRow(), self.table.currentColumn()] = float(self.table.currentItem().text().replace(',','.'))
        else: 
            self.master.master.kantine.iloc[self.table.currentRow(), self.table.currentColumn()] = self.table.currentItem().text()

        #print(self.master.master.kantine)
 
    def on_changed_combo(self):
        self.master.master.kantine.iloc[self.sender().property('Row'), 0] = self.sender().currentText()
    ...
    def delete_article(self):
        try:
            ...
            self.msgWarn.setText("Weet je zeker dat je " + curProdoms + " wilt verwijderen?")
            ...
            if retval == QMessageBox.Yes:
                ...
                self.master.master.kantine.drop(self.master.master.kantine[self.master.master.kantine.Productcode.astype(str) == curProdcode].index, inplace=True)
