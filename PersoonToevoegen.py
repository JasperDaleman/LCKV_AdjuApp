...
class PersoonToevoegen(QWidget):
    ...
    def conf(self):
        ...
        s = pd.Series(
            [name, gesl, func, tent, bc], index=self.master.master.personen.columns
        )

        try:
            self.master.master.personen = self.master.master.personen.append(
                s, ignore_index=True
            )
            ...
            if retval == QMessageBox.Yes:
                ...
                if ok:
                    ...
                    try:
                        inleg = float(inleg)
                        s2 = pd.Series(
                            [bc, inleg], index=self.master.master.inleg.columns
                        )
                        try:
                            self.master.master.inleg = self.master.master.inleg.append(
                                s2, ignore_index=True
                            )
