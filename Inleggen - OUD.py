...
class Inleggen(QWidget):
    ...
    def ingevoerd(self):
        if len(self.master.deelnemers_inleg) == 0:
            s = pd.Series(
                [self.bc.text(), int(self.inleg.text())], index=["Barcode", "Inleg"]
            )
            self.master.deelnemers_inleg = self.master.deelnemers_inleg.append(
                s, ignore_index=True, sort=False
            )
        else:
            idx = self.master.deelnemers_inleg[
                self.master.deelnemers_inleg.Barcode == self.bc.text()
            ]
            if len(idx) == 0:
                s = pd.Series(
                ...
                self.master.deelnemers_inleg.at[idx.index.values[0], "Inleg"] += float(
                    self.inleg.text()
                )
